...
async def _render_inbox(query, user_id: int, alias_id: str, *, note_new: bool = False) -> None:
    ...
    try:
        ...
        # get_alias_logs currently raises this deliberately — response
        # schema for reading mail content isn't confirmed yet (see the ⚠️
        # note in app/inboxmail.py). Not a bug, just a pending step.
        logger.info("get_alias_logs not ready for alias %s: %s", alias_id, exc)
        await query.edit_message_text(
            "🛠 <b>Inbox reading isn't wired up yet</b>\n\n"
