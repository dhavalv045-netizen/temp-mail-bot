...
## ⚠️ Current status (updated after live schema testing)

**✅ Working now:** creating aliases, deleting aliases, listing your
emails, all Telegram UI, OTP regex logic, rate-limit handling, security,
Render deployment. Confirmed live via "Try it out" in your Swagger docs:

```
POST /api/v1/aliases
{"domain_id": "<uuid>", "prefix": "...", "destinations": ["you@real-email.com"], "rule_type": "fixed"}
```
...
(the `destinations` field, required). To turn on "Create Email", add one
more environment variable in Render:

| Key | Value |
|---|---|
| `FORWARD_TO_EMAIL` | your own real email address |

**🛑 Paused — needs one more screenshot:** "📥 Inbox" and "🔢 Get OTP"
call `GET /api/v1/aliases/{id}/logs`, and it's not yet confirmed whether
those logs include the actual email content (from/subject/body) or only
forwarding metadata (delivered/bounced + timestamp). This decides whether
the bot can show mail *inside Telegram* at all, or whether it only forwards
to your real inbox. Both buttons currently show a clear "not wired up yet"
message instead of guessing.

**To unblock this:** in the docs, expand **"Get alias email logs"** under
Aliases, and screenshot either its **Schema** tab or the **200 response**
example (same place you found the alias schema) — no need to click
"Try it out" first, the documented example is enough. Send it over.
...
- **"Inbox" / "Get OTP" show a "not wired up yet" message**: expected until
  the alias-logs schema is confirmed — see the ⚠️ section at the top of
  this file.
